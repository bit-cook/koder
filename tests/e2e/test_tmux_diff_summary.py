...
async def _seed_session(home: Path, session_id: str) -> None:
    ...
    try:
        ...
        await session.add_items(
            [
                {"role": "user", "content": "Update auth file"},
                {
                    "toolUseResult": {
...
def test_tmux_diff_reports_conversation_edit_summary(tmp_path):
    ...
    home = tmp_path / "home"
    home.mkdir()
    session_id = "2026-04-09T13:00:00.000"
    asyncio.run(_seed_session(home, session_id))

    tmux_session = f"koder-diff-{uuid.uuid4().hex[:8]}"
    launch = " ".join(
        [
            "cd",
    ...
    try:
        ...
        _send(tmux_session, "/diff")
        output = _wait_for_output(tmux_session, "### Conversation edits")
        assert "## Diff" in output
        assert "### Conversation edits" in output
