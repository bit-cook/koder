...
async def _seed_session(home: Path, session_id: str, tracked_file: Path) -> None:
    ...
    try:
        ...
        await session.add_items(
            [
                {"role": "user", "content": "Investigate auth issue"},
                {
                    "role": "assistant",
                    "content": "Reviewing current auth files and repo guidance.",
                },
                {
                    "type": "function_call",
                    "name": "read_file",
...
def test_tmux_context_renders_usage_summary(tmp_path):
    ...
    session_id = "2026-04-09T12:30:00.000"
    asyncio.run(_seed_session(home, session_id, tracked))

    tmux_session = f"koder-context-{uuid.uuid4().hex[:8]}"
    launch = " ".join(
        [
            "cd",
    ...
    try:
        ...
        _send(tmux_session, "/context")
        output = _wait_for_output(tmux_session, "## Context Usage")
